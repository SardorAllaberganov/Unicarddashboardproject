      ...
      >
        ...
        <div style={{
          ...
          <FormSection title="Событие">
            ...
            <TextArea
              value={form.template}
              placeholder="Шаблон сообщения. Доступные переменные: {seller_name}, {card_number}, {kpi_step}, {amount}, {org_name}"
              onChange={v => setForm(p => ({ ...p, template: v }))}
            />
            <Caption>Переменные будут заменены реальными данными при отправке</Caption>
          </FormSection>
...
export default function NotificationRulesPage() {
  ...
  const openCreate = () => { setEditorMode('create'); setEditingRule(null); setEditorOpen(true); };
  const openEdit = (r: Rule) => { setEditorMode('edit'); setEditingRule(r); setEditorOpen(true); };
  const duplicate = (r: Rule) => { setEditorMode('create'); setEditingRule({ ...r, id: `${r.id}-copy` }); setEditorOpen(true); };

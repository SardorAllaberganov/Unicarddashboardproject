...
## 2026-04-20 — Vite on GitHub Pages: four places to thread the base path

Mistake: Deployed the SPA to GitHub Pages and got a blank page at `/Unicarddashboardproject/`. Setting `base: '/Unicarddashboardproject/'` in `vite.config.ts` was not enough — bundled assets loaded (correct URLs) but the router threw, icons 404'd, and the manifest scope was wrong.
Root cause: Vite's `base` only controls how it rewrites asset URLs in the built bundle. Four other consumers of the subpath needed wiring separately: (1) `createBrowserRouter` needs `basename` to strip the prefix before matching routes — otherwise `/Unicarddashboardproject/dashboard` is seen as a path it has no rule for. (2) PWA manifest `scope` and `start_url` default to `/`; on a subpath that breaks install + standalone-mode navigation. (3) `/favicon.svg` and `/splash/*.png` in `index.html` are absolute paths that Vite does NOT auto-prefix (it only rewrites `./relative` + `%BASE_URL%…` tokens). (4) GitHub Pages serves `404.html` on any unknown URL — SPA refreshes on `/dashboard` fail without a `404.html` fallback.
Fix: (1) `basename: import.meta.env.BASE_URL.replace(/\/$/, '')` in `createBrowserRouter` — the trailing slash must be stripped. (2) Set manifest `scope` and `start_url` to `base` in the Vite PWA config. (3) Change `/foo.png` to `%BASE_URL%foo.png` in `index.html`. (4) In the GH Actions workflow, `cp dist/index.html dist/404.html` after build. Also: toggle Repo Settings → Pages → Source to "GitHub Actions" once — the default "branch-based" Pages source ignores `deploy-pages@v4` entirely.
Rule: When deploying a Vite SPA under a subpath, drive the path from one env var (`BASE_PATH`) read by `vite.config.ts` and feed it to four consumers: Vite `base`, PWA manifest `scope` + `start_url`, `createBrowserRouter` `basename`, and `%BASE_URL%`-prefixed asset refs in `index.html`. Add the SPA 404 fallback in CI. Keep `pnpm dev` on `base: '/'` so local dev isn't surprised.

## 2026-04-20 — Git Bash / MSYS rewrites `/foo/` env values silently

Mistake: Tried to reproduce the production build locally on Windows with `BASE_PATH=/Unicarddashboardproject/ pnpm build`. The built `index.html` had `href="/Program%20Files/Git/Unicarddashboardproject/favicon.svg"` — Windows absolute path leaked into the deployed bundle.
Root cause: MSYS (Git Bash's POSIX layer on Windows) auto-translates arguments + env values that look like Unix paths. Anything starting with `/foo` gets rewritten to `C:\Program Files\Git\foo` before reaching the child process. `BASE_PATH=/Unicarddashboardproject/` is indistinguishable from a path literal to MSYS. CI on Linux is unaffected — this only bites on local Git Bash reproductions.
Fix: Prefix the command with `MSYS_NO_PATHCONV=1`. `MSYS_NO_PATHCONV=1 BASE_PATH=/Unicarddashboardproject/ pnpm build`. The env var disables path conversion for that process. Alternative: use a double-slash prefix `//Unicarddashboardproject/` which MSYS leaves alone but Vite reads as `/Unicarddashboardproject/`.
Rule: Any time you pass a forward-slash path value as an env var or CLI arg on Git Bash on Windows, wrap the command with `MSYS_NO_PATHCONV=1`. If the built output contains `/Program Files/Git/…` somewhere, that's MSYS — not your script. The gotcha doesn't affect pwsh, cmd, WSL, or Linux CI.

## 2026-04-20 — Mobile shell composition: double-headers + tab-bar clearance

Mistake: Two distinct shell-composition bugs shipped together. (1) Pages that drew their own 52 px sticky back-button header (AllCards, Sellers, SellerDetail, Notifications, NotificationRules, Settings, OrgSettings) also rendered the shared `<Navbar>` above them, stacking to ~108 px of chrome on mobile. (2) Page content containers used a bare `padding-bottom: 96px` to clear the MobileTabBar, but the tab bar is actually `calc(64px + env(safe-area-inset-bottom))` — ~98 px on iPhones with a home indicator — so the last content row was clipped by 2 px on notched devices.
Root cause: The responsive shell pattern renders `<Navbar />` in every page template, then conditionally swaps the desktop body for a mobile branch. Pages whose mobile branch is full-bleed (a detail, editor, or list with a custom top header) double-up on chrome because the Navbar keeps rendering. For bottom clearance, 96 px was a guess based on the **content** row of the tab bar (64 px) + breathing room, but ignored that safe-area stacks on top of that total.
Fix: (1) Added `hideOnMobile?: boolean` to `<Navbar>` — when true, returns `null` after all hooks run (respecting rules-of-hooks). Pages with their own sticky header pass `<Navbar hideOnMobile />`. (2) Standardized all bottom clearance to `calc(80px + env(safe-area-inset-bottom, 0px))` (64 px bar + 16 px breathing + safe-area). On Android / non-standalone web this collapses to 80 px; on notched iPhones it grows to ~114 px — always exactly clear of the tab bar.
Rule: Before rendering `<Navbar />` on a page, check whether the mobile branch draws its own sticky/fixed back-button header. If yes, pass `hideOnMobile`. For any mobile page rendered above the tab bar, pad the bottom scroll container with `calc(80px + env(safe-area-inset-bottom, 0px))` — never a bare `96px`, never `calc(96px + env(...))` (that's 2 px too tall on notched phones, and excess on Android). Early returns before hooks break the rules-of-hooks — always declare the conditional return after all `useState` / `useRef` / `useEffect` calls.

...
| [Navbar.tsx](../src/app/components/Navbar.tsx) | `<Navbar hideOnMobile? />`, `ORG_PATHS`, `detectRole()` | Role switcher, **theme toggle** (180° keyframe spin, lands back at 0° each click), notification bell with 4 states (`app:notif:new` / `app:notif:batch` CustomEvents), bell flyout, user menu. "Выйти из системы" menu item navigates to `/login`. Fully dark-themed across shell, bell dropdown, flyout, user menu, demo controls. `hideOnMobile` prop suppresses the 56 px mobile header on pages that draw their own sticky back-button header (AllCards, Sellers, SellerDetail, Notifications, NotificationRules, Settings, OrgSettings). |
...
| [ds/iconVariant.ts](../src/app/components/ds/iconVariant.ts) | `iconVariant(variant, dark)` | Dark-aware palette for tinted icon tiles (6 variants: blue/violet/green/cyan/amber/rose). Light palette = Tailwind 50/600 pairs; dark palette = 15 % alpha washes for consistent contrast on `#1A1D27`. Used by BankAdminDashboardPage, OrgDetailPage, and mobile dashboard row components. |
...
- **Mobile double-header fix.** Pages with their own sticky back-button header (52 px) must pass `hideOnMobile` to `<Navbar>` — otherwise the 56 px mobile navbar stacks on top producing a ~108 px chrome. Applies to AllCards, Sellers, SellerDetail, Notifications, NotificationRules, Settings, OrgSettings. Dashboards / top-level tabs keep the default Navbar (they don't draw their own header).
- **Tab-bar clearance padding.** Pages rendered above the MobileTabBar must pad their bottom with `calc(80px + env(safe-area-inset-bottom, 0px))` (64 px bar + 16 px breathing + safe-area). A bare `96px` under-covers iPhones with the home indicator by a few px.
- **iOS PWA splash screens.** 20 PNGs in `public/splash/` (10 iPhone sizes × light/dark) wired into `index.html` via `<link rel="apple-touch-startup-image" media="...">`. Regenerate with `node scripts/gen-splash-screens.mjs`. The launch image is a centered 22 %-of-shortest-side icon on `#F9FAFB` (light) / `#0B0D14` (dark).
